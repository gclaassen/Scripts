...
while true; do
    ...
    case $yn in
        [Yy]* ) sudo snap install ohmygiraffe; sudo snap install boa; sudo snap install gnome-clocks; sudo snap install vlc; sudo snap install qaclculate; sudo snap install syncthing; sudo snap install inkscape; sudo snap install telegram-desktop; sudo snap install whatsapp-for-linux; sudo apt install -y meld cryptomator; flatpak install -y zotero; sudo apt install -y libavcodec-extra libdvd-pkg; sudo dpkg-reconfigure libdvd-pkg;  break;;
        [Nn]* ) break;;
        * ) echo "Please answer yes or no.";;
    esac
done
