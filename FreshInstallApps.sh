...
while true; do
    ...
    case $yn in
        [Yy]* ) sudo snap install ohmygiraffe; sudo snap install yakyak; sudo snap install boa; sudo snap install gnome-clocks; sudo snap install vlc; sudo snap install qaclculate; sudo snap install syncthing; sudo snap install inkscape; sudo snap install telegram-desktop; sudo snap install whatsapp-for-linux; sudo apt install -y speedcrunch qalculate cryptomator zsh; flatpak install -y zotero; sudo apt install -y libavcodec-extra libdvd-pkg; sudo dpkg-reconfigure libdvd-pkg; cd ~/Downloads; sh -c "$(wget https://raw.github.com/ohmyzsh/ohmyzsh/master/tools/install.sh -O -)";  break;;
        [Nn]* ) break;;
        * ) echo "Please answer yes or no.";;
    esac
done
